...
def analyse_and_persist(asin_obj: ASIN, raw_reviews: list[dict]) -> ReviewAnalysis:
    ...
    negative_texts = _filter_by_rating(raw_reviews, max_rating=2)
    ...
    positive_themes = _extract_themes(
        [_clean_text(r["text"]) for r in positive_texts], 
        context_words=context_words
    )
    negative_themes = _extract_themes(
...
def _extract_themes(texts: List[str], top_n: int = 5, context_words: Optional[List[str]] = None) -> List[str]:
    """
    Extract top keyword themes using frequency-based analysis.
    """
    if not texts:
        return []

    combined = " ".join(texts[:200])  # cap input size

    # NOTE: KeyBERT is disabled due to environment-specific hangs.
    # We use frequency-based theme extraction instead.
    return _frequency_themes(combined, top_n)


def _frequency_themes(text: str, top_n: int = 5) -> list[str]:
    """Simple word-frequency fallback when KeyBERT is unavailable."""
    from collections import Counter

    STOP = {"the", "a", "an", "is", "it", "i", "my", "this", "and", "or", "but", "very", "so", "not", "was", "for", "in", "on", "to", "of", "with", "they", "are", "have", "these", "that", "you", "your", "them", "from", "at", "as", "be", "their", "just", "me", "get", "got", "can", "out", "like", "up", "all"}
